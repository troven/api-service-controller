...
class WatcherPlugin {
    ...
    install(context, _options) {
        ...
        _.each(this.options, (option, name) => __awaiter(this, void 0, void 0, function* () {
            ...
            context.log({ code: "watcher:install", name: name, options: option, gvk: watcher.gvk() });
    ...
    install_crd(context, crd) {
        ...
        context.log({ code: "api:k8s:watch:crd:install", crd: crd });
        return new Promise((resolve, reject) => {
            this.createCRD(this.kc, crd).then((crd) => {
                ...
                if (err.response && err.response.body) {
                    ...
                    if (err.code == 409) {
                        context.log({ code: "api:k8s:watch:crd:exist", message: err ? err.message : "", kind: crd.kind });
                        return resolve(crd);
                    }
                    else
                        reject({ code: "api:k8s:watch:crd:failed", message: err ? err.message : "", error: err, kind: crd.kind });
                }
                else {
                    reject({ code: "api:k8s:watch:crd:error", message: err ? err.message : "", error: err, kind: crd.kind });
                }
            });
        });
    }
...
exports.WatcherPlugin = WatcherPlugin;
//# sourceMappingURL=WatcherPlugin.js.map